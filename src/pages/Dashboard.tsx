...
import PomodoroCard, { type PomodoroCardHandle } from '@/components/dashboard/PomodoroCard'
...
const Dashboard = () => {
  ...
  const pomodoroRef = useRef<PomodoroCardHandle>(null)

  return (
    <div className="dot-grid flex h-dvh flex-col overflow-y-auto overflow-x-hidden">
      <AppNavbar running={running} pauseTimer={() => pomodoroRef.current?.pauseTimer()} />
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center px-4 py-6 pb-24 sm:px-6 lg:pb-50">
        <div className="grid min-h-0 gap-4 lg:grid-cols-[minmax(0,400px)_minmax(0,1fr)]">
          <PomodoroCard ref={pomodoroRef} running={running} setRunning={setRunning} mode={mode} setMode={setMode} remaining={remaining} setRemaining={setRemaining} selectedTaskId={selectedTaskId} setSelectedTaskId={setSelectedTaskId} />
          <div className="relative min-w-0">
            <TasksPanel selectedTaskId={selectedTaskId} selectTask={(id) => pomodoroRef.current?.selectTask(id)} />
          </div>
        </div>
